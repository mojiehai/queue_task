...
class RedisDrive {
    ...
    protected function open(){
        ...
        if(self::$socket) {
            ...
            throw new DBException("Failed to open redis DB connection (".$connection."): ".$errorNumber);
        }
    }
    ...
    /**
     * 执行redis命令
     * @param String $name      执行的命令
     * @param array $params     执行的参数，按照顺序
     * @return mixed
     * @throws DBException
     * @see https://redis.io/topics/protocol
     *
     * 客户端发送命令LLEN mylist以获取存储在密钥mylist的列表的长度，并且服务器使用Integer回复进行回复
     * 如以下示例（C：客户端，S：服务器）。
     * C: *2\r\n
     * C: $4\r\n
     * C: LLEN\r\n
     ...
     *
     *
     * 通常我们为了简单起见，将协议的不同部分与换行分开，但是实际的交互是客户端发送
     * *2\r\n$4\r\nLLEN\r\n$6\r\nmylist\r\n的整体。
     */
    ...
    protected function parseResponse($command)
    {
        ...
        switch ($type) {
            case '+': // Status reply       对于简单字符串，回复的第一个字节是“+”
            ...
            case ':': // Integer reply      对于整数，回复的第一个字节是“：”
                ...
                return $line;
            case '$': // Bulk replies       对于批量字符串，答复的第一个字节是“$”，(每行用\r\n结尾)，第一行的表示所有字符串长度之和
                if ($line == '-1') {
                    return null;
                }
            ...
            case '*': // Multi-bulk replies     对于数组，答复的第一个字节是“ *”
                $count = (int) $line;
                $data = [];
                for ($i = 0; $i < $count; $i++) {
    ...
    public function __call($name, $params)
    {
        ...
        if (in_array($redisCommand, $this->redisCommands)) {
            ...
            throw new DBException("Redis Command Error: '".$redisCommand."' is not found");
        }
    }
